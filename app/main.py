...
def list_coins(limit:int=Query(100,le=500),min_score:float=0.0,db:Session=Depends(get_session)):
    ...
    protocol_by_id={coin_id:profile.get("protocol_research") for coin_id,profile in research_by_id.items()}
